...
function setup() {
  ...
  buttonReset.position(1440 / 25, 3 * 737 / 20);
...
function draw() {
  ...
  if (x[0] === 0 && y[0]) {
    text('a = ' + y[0].toFixed(2) + 'i',
          5 * 1440 / 6, (firstVertexLabelNumber + 1.5) * 737 / 20 + 2);
  } else if (x[0] && y[0]) {
  ...
  if (x[1] === 0 && y[1]) {
    text('b = ' + y[1].toFixed(2) + 'i',
          5 * 1440 / 6, (secondVertexLabelNumber + 1.5) * 737 / 20 + 2);
  } else if (x[1] && y[1]) {
  ...
  text('CLICK AND DRAG THE POINTS TO', 2 * 1440 / 7, 737 / 11);
  text('MOVE THEM AROUND THE HALF-PLANE', 2 * 1440 / 7, 737 / 11 + 20);
  pop();
}
