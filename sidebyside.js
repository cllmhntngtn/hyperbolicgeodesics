...
function draw() {
  ...
  var circleCentrePD = (x[0] ** 2 + y[0] ** 2 - x[1] ** 2 - y[1] ** 2) / (2 * x[0] - 2 * x[1]);
  var circleRadiusPD = dist(x[0], y[0], circleCentrePD, 0);
  // left point
  var leftPointX = circleCentrePD - circleRadiusPD;
  let leftPointUHP = new Complex(leftPointX, 0);
  let leftPointPD = (leftPointUHP.sub(Complex['I'])).div(Complex['I'].mul(leftPointUHP).sub(1));
  // rightPoint
  var rightPointX = circleCentrePD + circleRadiusPD;
  ...
  // drawing the geodesics in the disc
  push();
  strokeWeight(6);
  stroke('dodgerblue')
  noFill();
  ...
  // calculating the relevant semi-circle for half-plane
  var circleCentreUHP = (x[0] ** 2 + y[0] ** 2 - x[1] ** 2 - y[1] ** 2) / (2 * x[0] - 2 * x[1]);
  var circleRadiusUHP = dist(x[0], y[0], circleCentreUHP, 0);
  // calculating angles for the arc along the semi-circle for half-plane
  var alphaUHP = acos((- circleRadiusUHP * (x[0] - circleCentreUHP)) / (circleRadiusUHP
                  * sqrt((x[0] - circleCentreUHP) ** 2 + y[0] ** 2)));
  var betaUHP = acos((- circleRadiusUHP * (x[1] - circleCentreUHP)) / (circleRadiusUHP
                  * sqrt((x[1] - circleCentreUHP) ** 2 + y[1] ** 2)));

  // drawing the geodesics in the half-plane
  push();
  strokeWeight(6);
  strokeCap(SQUARE);
  noFill();
  if (x[0] === x[1] && y.length === 2) {
    ...
    line(xOriginUHP + x[0] * xStepUHP, yOriginUHP,
      xOriginUHP + x[1] * xStepUHP, yMaxUHP);
    stroke('dodgerblue');
    line(xOriginUHP + x[0] * xStepUHP, yOriginUHP - y[0] * yStepUHP,
      xOriginUHP + x[1] * xStepUHP, yOriginUHP - y[1] * yStepUHP);
  } else if (x[0] < x[1]) {
    stroke('darkgray');
    arc(xOriginUHP + circleCentreUHP * xStepUHP, yOriginUHP,
      2 * circleRadiusUHP * xStepUHP, 2 * circleRadiusUHP * yStepUHP, PI, 0, OPEN);
    stroke('dodgerblue');
    arc(xOriginUHP + circleCentreUHP * xStepUHP, yOriginUHP,
      2 * circleRadiusUHP * xStepUHP, 2 * circleRadiusUHP * yStepUHP,
