...
function draw() {
  ...
  const yOrigin = 7 * windowHeight / 15;
  const discRadius = 2 * windowHeight / 5;

  // radius of the unit circle is 2 * windowHeight / 5
  ...
  var a = [xOrigin + hz_1.re * discRadius, yOrigin - hz_1.im * discRadius];
  var b = [xOrigin + hz_2.re * discRadius, yOrigin - hz_2.im * discRadius];
  var c = [xOrigin + leftPointPD.re * discRadius, yOrigin - leftPointPD.im * discRadius];
  ...
  if (x[0] === '0' && x[1] === '0' && x.length === 2 && y.length === 2) {
    ...
    stroke('dodgerblue');
    line(xOrigin + hz_1.re * discRadius, yOrigin - hz_1.im * discRadius,
          xOrigin + hz_2.re * discRadius, yOrigin - hz_2.im * discRadius);
  }
  pop();

  // TRYING TO WORK OUT HOW TO DRAW GEODESICS WHEN TWO POINTS LIE ON DIAMETER. I
  // WANT SOMETHING WHERE IT CAN CALCULATE THE POINTS WHERE A STRAIGHT LINE WHICH
  // PASSES THROUGH THE TWO POINTS WILL INTERSECT THE BOUNDARY OF THE DISC AND
  // THEN CHECK TO SEE IF THE DISTANCE BETWEEN THE TWO POINTS ON THE BOUNDARY IS
  // EQUAL TO THE DIAMETER OF THE DISC

  //var gradient = ((yOrigin + hz_2.im * discRadius) - (yOrigin + hz_1.im * discRadius))
                  // ((xOrigin + hz_2.re * discRadius) - (xOrigin + hz_1.re * discRadius));
  //var c = y[0] + gradient * x[0];

  //if (x.length === 2 && y.length === 2) {
  //  text(c, xOrigin, yOrigin);
  //}
  ...
  //if (x.length === 2 && y.length === 2) {
  //  stroke('darkgray');
  //  line(0, c, windowWidth, - gradient * windowWidth + c);
  //  stroke('dodgerblue');
  //  line(xOrigin + hz_1.re * discRadius, yOrigin - hz_1.im * discRadius,
  //        xOrigin + hz_2.re * discRadius, yOrigin - hz_2.im * discRadius);
  //}
  ...
  fill('white');
  if (x[0] && y[0]) {
      circle(xOrigin + hz_1.re * discRadius, yOrigin - hz_1.im * discRadius, 25);
  }
  if (x[1] && y[1]) {
      circle(xOrigin + hz_2.re * discRadius, yOrigin - hz_2.im * discRadius, 25);
  }
  fill('black');
  textFont('Helvetica');
  textSize(20);
  if (x[0] && y[0]) {
    text(vertexLabels[firstVertexLabelNumber],
      xOrigin + hz_1.re * discRadius, yOrigin - hz_1.im * discRadius);
  }
  if (x[1] && y[1]) {
  ...
  beginShape();
    ...
    beginContour();
      for(var k = 0; k<=N; k++) {
        vertex(xOrigin + discRadius * cos(-k * 2 * PI/N),
          yOrigin + discRadius * sin(-k * 2 * PI/N));
      }
    endContour();
  endShape();
  pop();
  ...
  strokeWeight(2);
  noFill();
  circle(xOrigin, yOrigin, 2 * discRadius);
  pop();
